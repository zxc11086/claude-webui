...
export function useWebSocket() {
  ...
  useEffect(() => {
    ...
    socket.on('session.resumed', (data: { sessionId: string; messages: any[] }) => {
      ...
      state.loadSessionMessages(data.sessionId, messages);
      
      // Switch to this session
      state.setActiveSession(data.sessionId);
    });
    ...
    socket.on('assistant.delta', (data: { sessionId: string; delta: string }) => {
      store.getState().appendDelta(data.sessionId, data.delta);
    });

    socket.on('assistant.completed', (data: { sessionId: string }) => {
      store.getState().finishStreaming(data.sessionId);
    });

    // --- Tool calls ---
    socket.on('tool.call', (data: { sessionId: string; toolId: string; tool: string; input: any }) => {
      store.getState().addToolCall(data.sessionId, {
        toolId: data.toolId,
        tool: data.tool,
        input: data.input,
        status: 'pending',
    ...
    socket.on('tool.stdout', (data: { sessionId: string; toolId: string; stream: string; delta: string }) => {
      const state = store.getState();
      state.updateToolCall(data.sessionId, data.toolId, { status: 'running' });
      state.appendToolOutput(
        data.sessionId,
        data.toolId,
    ...
    socket.on('tool.stderr', (data: { sessionId: string; toolId: string; delta: string }) => {
      const state = store.getState();
      state.updateToolCall(data.sessionId, data.toolId, { status: 'running' });
      state.appendToolOutput(data.sessionId, data.toolId, 'stderr', data.delta);
    });
    ...
    socket.on('tool.result', (data: { sessionId: string; toolId: string; result: any }) => {
      ...
      state.updateToolCall(data.sessionId, data.toolId, {
        status: 'completed',
        output: resultStr,
      });
    ...
    socket.on('file.updated', (data: { sessionId: string; path: string; patch: string }) => {
      store.getState().addMessage(data.sessionId, {
        id: `file-${Date.now()}`,
        role: 'tool',
